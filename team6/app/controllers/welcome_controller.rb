...
#  @trains = [
#          { :id => 0, :color => 'train', :coordinates => { :lat => 50.464853086055115, :lng => 30.384784727096558100 }, :name => "" },
#          { :id => 1, :color => 'train', :coordinates => { :lat => 50.45618571221397, :lng => 30.363775491714478 }, :name => "" },
#          { :id => 2, :color => 'train', :coordinates => { :lat => 50.457620139071935, :lng => 30.40191722869873 }, :name => "" },
#          { :id => 3, :color => 'train', :coordinates => { :lat => 50.45892474668018, :lng => 30.504738187789917 }, :name => "" }
#   ]
  @trains = ::SubwayData.new.get_connection_info;
  ...
  @stations.each do |station|
    @markers_data[:markers].push({:lat => station[:coordinates][:lat], :lng => station[:coordinates][:lng], :html => station[:name], :label => station[:name], :color => station[:color]})
  end

  @trains.each do |train|
    @trains_data[:markers].push({:lat => train[:coordinates][:lat], :lng => train[:coordinates][:lng], :html => train[:name], :label => train[:name], :color => train[:color]})
  end
    
  end
  def station_tables
    @stations = StationsTable.simple_table(::SubwayData.new.get_station_info)    
  end
end
